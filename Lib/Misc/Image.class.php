...
{//类定义开始
     ...
     * @return void
     */
    static function cut($image,$cutfile,$cutWidth='',$cutHeight='', $sharpen = false){
        ...
        if($info){
            ...
            empty($cutWidth) && $cutWidth = $srcWidth;  //切割后的宽
            empty($cutHeight) && $cutHeight = $srcHeight; //切割后的高
            
            $swh = $srcWidth/$srcHeight;
            $cwh = $cutWidth/$cutHeight;

            //>> start 方案3：按照目标图比例切割，再压缩
            if ($swh > $cwh) {
                $_srcWidth  = ~~($srcHeight * $cutWidth / $cutHeight);
                $_srcHeight = $srcHeight;
                $_s_x = ~~(($srcWidth - $_srcWidth) / 2);
                $_s_y = 0;
            } else {
                $_srcWidth  = $srcWidth;
                $_srcHeight = ~~($srcWidth * $cutHeight / $cutWidth);
                $_s_x = 0;
                $_s_y = ~~(($srcHeight - $_srcHeight) / 2);
            }
            //<< end 方案3
            ...
            // 使用ImageMagick 进行处理
            @exec("convert {$image} "
            . "-crop {$_srcWidth}x{$_srcHeight}+{$_s_x}+{$_s_y} "
            . "-resize {$cutWidth}x{$cutHeight} "
            . ($sharpen ? "-sharpen .001 " : "")
            // . "-sharpen .001 "
            . "{$cutfile}");

            // 处理成功，则返回缩略图路径，否则继续执行php 的图片处理
     ...
     * @return void
     +----------------------------------------------------------
     */
    static function thumb($image,$thumbname,$type='',$maxWidth=200,$maxHeight='auto', $sharpen = false, $interlace=true)
    {
        // 获取原图信息
        $info  = Image::getImageInfo($image);
         if($info !== false) {
