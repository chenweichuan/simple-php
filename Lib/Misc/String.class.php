...
class String
{
    static public function getStringFixedLength($string, $strip_html = FALSE)
    {
        if ($strip_html) {
    		$string = html_entity_decode($string, ENT_QUOTES);
    		$string = strip_tags($string);
    	}
    	$length = (strlen($string) + mb_strlen($string, 'UTF8')) / 4;
    	return intval(ceil($length));
    }
	...
	static public function rand_string($len=6,$type='',$addChars='') {
		...
		if($type!=4) {
			...
			// 中文随机字
			for($i=0;$i<$len;$i++){
			  $str.= self::msubstr($chars, ~~(mt_rand(0,mb_strlen($chars,'utf-8')-1)),1);
			}
		}
		return $str;
	}
